...
Replace runtime Bun shell template-tag usage in `packages/kx/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/kx/src` is removed except:
