import type { Hooks, PluginInput, Plugin as PluginInstance } from "@kx/plugin"
import { Config } from "../config/config"
import { Bus } from "../bus"
import { Log } from "../util/log"
import { createKxClient } from "@kx/sdk"
        ...
        Effect.fn("Plugin.state")(function* (ctx) {
          ...
          yield* Effect.promise(async () => {
            const client = createKxClient({
              ...
              headers: Flag.KX_SERVER_PASSWORD
                ? {
                    Authorization: `Basic ${Buffer.from(`${Flag.KX_SERVER_USERNAME ?? "kx"}:${Flag.KX_SERVER_PASSWORD}`).toString("base64")}`,
                  }
                : undefined,
