...
export namespace LSP {
  ...
  const filterExperimentalServers = (servers: Record<string, LSPServer.Info>) => {
    if (Flag.KX_EXPERIMENTAL_LSP_TY) {
      // If experimental flag is enabled, disable pyright
      if (servers["pyright"]) {
        log.info("LSP server pyright is disabled because KX_EXPERIMENTAL_LSP_TY is enabled")
  ...
  export async function touchFile(input: string, waitForDiagnostics?: boolean) {
    ...
    await Promise.all(clients.map((client) => client.notify.open({ path: input, waitForDiagnostics }))).catch((err) => {
  ...
  export namespace Diagnostic {
    ...
    export function pretty(diagnostic: LSPClient.Diagnostic, id?: string) {
      return `${label(diagnostic)} ${ref(diagnostic, id)} ${diagnostic.message}${suggestions(diagnostic)}`
    }
  }
}
