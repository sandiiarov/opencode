...
async function resolveRelative(instruction: string): Promise<string[]> {
  ...
  if (!Flag.KX_CONFIG_DIR) {
    log.warn(`Skipping relative instruction "${instruction}" - no KX_CONFIG_DIR set while project config is disabled`)
    return []
  }
  return Filesystem.globUp(instruction, Flag.KX_CONFIG_DIR, Flag.KX_CONFIG_DIR).catch(() => [])
}
