...
import type { AssistantMessage, Part, Provider, ToolPart, UserMessage, TextPart, ReasoningPart } from "@kx/sdk/v2"
...
export function Session() {
  ...
  createEffect(() => {
    ...
    const logo = UI.logo("  ", { solid: true }).split(/\r?\n/)
    return exit.message.set(
      [
        ...
        `  ${weak("Continue")}${UI.Style.TEXT_NORMAL_BOLD}kx -s ${session()?.id}${UI.Style.TEXT_NORMAL}`,
        ``,
      ].join("\n"),
    )
  })
  ...
  return (
    <Show when={props.part.text.trim()}>
      <Pane id={"text-" + props.part.id} color={tone(theme, BLOCK_TONE.assistant.text)}>
        <Switch>
          <Match when={Flag.KX_EXPERIMENTAL_MARKDOWN}>
            <markdown
              syntaxStyle={syntax()}
              streaming={true}
