...
import kx from "./theme/kx.json" with { type: "json" }
...
export const { use: useTheme, provider: ThemeProvider } = createSimpleContext({
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    const [store, setStore] = createStore({
      ...
      lock,
      active: (config.theme ?? kv.get("theme", "kx")) as string,
      ready: false,
    })
    ...
    const values = createMemo(() => {
      return resolveTheme(store.themes[store.active] ?? store.themes.kx, store.mode)
    })
