...
const TIPS = [
  ...
  "Run {highlight}/share{/highlight} to create a public link to your conversation at kx.ai",
  ...
  "Create {highlight}kx.json{/highlight} for server settings and {highlight}tui.json{/highlight} for TUI settings",
  "Place TUI settings in {highlight}~/.config/kx/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Add {highlight}.md{/highlight} files to {highlight}.kx/command/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.kx/agent/{/highlight} for specialized AI personas",
  "Configure per-agent permissions for {highlight}edit{/highlight}, {highlight}bash{/highlight}, and {highlight}webfetch{/highlight} tools",
  'Use patterns like {highlight}"git *": "allow"{/highlight} for granular bash permissions',
  'Set {highlight}"rm -rf *": "deny"{/highlight} to block destructive commands',
  'Configure {highlight}"git push": "ask"{/highlight} to require approval before pushing',
  "kx auto-formats files using prettier, gofmt, ruff, and more",
  'Set {highlight}"formatter": false{/highlight} in config to disable all auto-formatting',
  "Define custom formatter commands with file extensions in config",
  "kx uses LSP servers for intelligent code analysis",
  "Create {highlight}.ts{/highlight} files in {highlight}.kx/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.kx/plugin/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent kx from reading sensitive files",
  "Use {highlight}kx run{/highlight} for non-interactive scripting",
  "Use {highlight}kx --continue{/highlight} to resume the last session",
  "Use {highlight}kx run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}kx serve{/highlight} for headless API access to kx",
  "Use {highlight}kx run --attach{/highlight} to connect to a running server",
  "Run {highlight}kx uninstall{/highlight} to remove local kx data and binaries",
  "Run {highlight}kx auth list{/highlight} to see all configured providers",
  "Run {highlight}kx agent create{/highlight} for guided agent creation",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.kx/themes/{/highlight} directory",
  ...
  "Run {highlight}kx debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/anomalyco/kx{/highlight} for containerized use",
  "Use {highlight}/connect{/highlight} with kx Zen for curated, tested models",
