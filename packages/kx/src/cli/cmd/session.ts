...
function pagerCmd(): string[] {
  ...
  if (Flag.KX_GIT_BASH_PATH) {
    const less = path.join(Flag.KX_GIT_BASH_PATH, "..", "..", "usr", "bin", "less.exe")
...
export const SessionCommand = cmd({
  ...
  builder: (yargs: Argv) =>
    yargs.command(SessionListCommand).command(SessionTranscriptCommand).command(SessionDeleteCommand).demandCommand(),
  async handler() {},
})
