...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run kx",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for kx session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found kx session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["kx", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting kx...")
        UI.println()

        const kxArgs = sessionId ? ["-s", sessionId] : []
        const kxProcess = Process.spawn(["kx", ...kxArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await kxProcess.exited
        if (code !== 0) throw new Error(`kx exited with code ${code}`)
      },
    })
  },
})
