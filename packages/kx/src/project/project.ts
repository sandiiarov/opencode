...
export namespace Project {
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@kx/Project") {}
  ...
  > = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const fakeVcs = Info.shape.vcs.parse(Flag.KX_FAKE_VCS)
      ...
      const readCachedProjectId = Effect.fnUntraced(function* (dir: string) {
        return yield* fsys.readFileString(pathSvc.join(dir, "kx")).pipe(
          Effect.map((x) => x.trim()),
      ...
      const fromDirectory = Effect.fn("Project.fromDirectory")(function* (directory: string) {
        ...
        const data: DiscoveryResult = yield* Effect.gen(function* () {
          ...
          if (!id) {
            ...
            if (id) {
              yield* fsys.writeFileString(pathSvc.join(worktree, ".git", "kx"), id).pipe(Effect.ignore)
            }
          }
        ...
        if (Flag.KX_EXPERIMENTAL_ICON_DISCOVERY) yield* discover(existing).pipe(Effect.ignore, Effect.forkIn(scope))
