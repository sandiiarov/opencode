  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    async kx(input) {
      const hasKey = await (async () => {
        ...
        if (config.provider?.["kx"]?.options?.apiKey) return true
        return false
      })()
    ...
    "amazon-bedrock": async () => {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from kx.json provider config
    ...
    gitlab: async (input) => {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `kx/${Installation.VERSION} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `kx auth cloudflare-ai-gateway`.",
        )
      }
    ...
    cerebras: async () => {
      return {
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "kx",
          },
        },
      }
    },
  ...
  const state = Instance.state(async () => {
    ...
    for (const [id, provider] of Object.entries(providers)) {
      ...
      for (const [modelID, model] of Object.entries(provider.models)) {
        ...
        if (model.status === "alpha" && !Flag.KX_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
