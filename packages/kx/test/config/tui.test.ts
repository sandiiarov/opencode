...
afterEach(async () => {
  delete process.env.KX_CONFIG
  delete process.env.KX_TUI_CONFIG
...
test("loads tui config with the same precedence order as server config paths", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await fs.mkdir(path.join(dir, ".kx"), { recursive: true })
      await Bun.write(
        path.join(dir, ".kx", "tui.json"),
...
test("ignores tui-specific keys in kx.json", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(config.keybinds?.app_exit).not.toBe("ctrl+q")
    },
  })
})

test("project config takes precedence over KX_TUI_CONFIG (matches KX_CONFIG)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.KX_TUI_CONFIG = custom
    },
  })
...
test("KX_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
...
test("does not derive tui path from KX_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const customDir = path.join(dir, "custom")
      await fs.mkdir(customDir, { recursive: true })
      await Bun.write(path.join(customDir, "kx.json"), JSON.stringify({ model: "test/model" }))
      await Bun.write(path.join(customDir, "tui.json"), JSON.stringify({ theme: "should-not-load" }))
      process.env.KX_CONFIG = path.join(customDir, "kx.json")
    },
  })
...
test("loads .kx/tui.json", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.mkdir(path.join(dir, ".kx"), { recursive: true })
      await Bun.write(path.join(dir, ".kx", "tui.json"), JSON.stringify({ diff_style: "stacked" }, null, 2))
    },
  })
