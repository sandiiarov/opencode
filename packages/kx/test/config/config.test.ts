...
const managedConfigDir = process.env.KX_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "kx.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "kx.json") {
...
test("ignores legacy tui keys in kx config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://kx.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "kx.json"))
...
test("resolves env templates in account config with account token", async () => {
  ...
  const originalControlToken = process.env["KX_CONSOLE_TOKEN"]
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        const config = await Config.get()
        expect(config.provider?.["kx"]?.options?.apiKey).toBe("st_test_token")
      },
    })
  } finally {
    ...
    if (originalControlToken !== undefined) {
      process.env["KX_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["KX_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "kx.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .kx directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const kxDir = path.join(dir, ".kx")
      await fs.mkdir(kxDir, { recursive: true })
      const agentDir = path.join(kxDir, "agent")
...
test("loads agents from .kx/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const kxDir = path.join(dir, ".kx")
      await fs.mkdir(kxDir, { recursive: true })

      const agentsDir = path.join(kxDir, "agents")
...
test("loads commands from .kx/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const kxDir = path.join(dir, ".kx")
      await fs.mkdir(kxDir, { recursive: true })

      const commandDir = path.join(kxDir, "command")
...
test("loads commands from .kx/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const kxDir = path.join(dir, ".kx")
      await fs.mkdir(kxDir, { recursive: true })

      const commandsDir = path.join(kxDir, "commands")
...
test("does not try to install dependencies in read-only KX_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.KX_CONFIG_DIR
  process.env.KX_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.KX_CONFIG_DIR
    else process.env.KX_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable KX_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "kx.json"),
        JSON.stringify({ $schema: "https://kx.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "kx.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .kx config
      const projectDir = path.join(dir, "project")
      const kxDir = path.join(projectDir, ".kx")
      await fs.mkdir(kxDir, { recursive: true })
      ...
      // Local .kx config with different plugins
      await Filesystem.write(
        path.join(kxDir, "kx.json"),
        JSON.stringify({
          $schema: "https://kx.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .kx config with some overlapping plugins
      await Filesystem.write(
        path.join(kxDir, "kx.json"),
        JSON.stringify({
          $schema: "https://kx.ai/config.json",
...
// Note: preload.ts sets KX_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-kx@2.4.3")).toBe("oh-my-kx")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@kx/plugin@2.0.0")).toBe("@kx/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-kx@2.4.3", "file:///project/.kx/plugin/oh-my-kx.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.kx/plugin/oh-my-kx.js")
  })
  ...
  test("local plugin directory overrides global kx.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const kxDir = path.join(projectDir, ".kx")
        const pluginDir = path.join(kxDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("KX_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["KX_DISABLE_PROJECT_CONFIG"]
    process.env["KX_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["KX_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["KX_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .kx/ directories when flag is set", async () => {
    const originalEnv = process.env["KX_DISABLE_PROJECT_CONFIG"]
    process.env["KX_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .kx directory with a command
          const kxDir = path.join(dir, ".kx", "command")
          await fs.mkdir(kxDir, { recursive: true })
          await Filesystem.write(path.join(kxDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .kx should NOT be in directories list
          const hasProjectKx = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectKx).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["KX_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["KX_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["KX_CONFIG_DIR"]
      process.env["KX_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["KX_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["KX_CONFIG_DIR"]
      } else {
        process.env["KX_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("KX_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["KX_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["KX_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["KX_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["KX_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from KX_CONFIG_DIR, not project
...
describe("KX_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in KX_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["KX_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["KX_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://kx.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["KX_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["KX_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in KX_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["KX_CONFIG_CONTENT"]
