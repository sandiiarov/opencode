...
describe("tool.skill", () => {
  test("description lists skill location URL", async () => {
    await using tmp = await tmpdir({
      git: true,
      init: async (dir) => {
        const skillDir = path.join(dir, ".kx", "skill", "tool-skill")
        await Bun.write(
    ...
    try {
      await Instance.provide({
        ...
        fn: async () => {
          ...
          const skillPath = path.join(tmp.path, ".kx", "skill", "tool-skill", "SKILL.md")
    ...
    await using tmp = await tmpdir({
      ...
      init: async (dir) => {
        for (const [name, description] of [
          ...
          const skillDir = path.join(dir, ".kx", "skill", name)
          await Bun.write(
            path.join(skillDir, "SKILL.md"),
            `---
name: ${name}
    ...
    try {
      await Instance.provide({
        ...
        fn: async () => {
          ...
          const dir = path.join(tmp.path, ".kx", "skill", "tool-skill")
