...
function tool(sessionID: string, messageID: string) {
  return Session.updatePart({
    ...
    tool: "bash",
    callID: "call-1",
    state: { status: "completed", input: {}, output: "done", title: "", metadata: {}, time: { start: 0, end: 1 } },
  })
}
  ...
  test("restore messages in sequential order", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        await SessionRevert.revert({ sessionID: sid, messageID: first })
