...
test("discovers skills from .kx/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".kx", "skill", "test-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: test-skill
...
test("returns skill directories from Skill.dirs", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".kx", "skill", "dir-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: dir-skill
...
`,
  ...
  const home = process.env.KX_TEST_HOME
  process.env.KX_TEST_HOME = tmp.path
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const skillDir = path.join(tmp.path, ".kx", "skill", "dir-skill")
        expect(dirs).toContain(skillDir)
        expect(dirs.length).toBe(1)
      },
    })
  } finally {
    process.env.KX_TEST_HOME = home
  }
})

test("discovers multiple skills from .kx/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir1 = path.join(dir, ".kx", "skill", "skill-one")
      const skillDir2 = path.join(dir, ".kx", "skill", "skill-two")
      await Bun.write(
...
test("skips skills with missing frontmatter", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".kx", "skill", "no-frontmatter")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `# No Frontmatter
...
test("discovers global skills from ~/.claude/skills/ directory", async () => {
  ...
  const originalHome = process.env.KX_TEST_HOME
  process.env.KX_TEST_HOME = tmp.path
  ...
  try {
    ...
    process.env.KX_TEST_HOME = originalHome
  }
})
...
test("properly resolves directories that skills live in", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const kxSkillDir = path.join(dir, ".kx", "skill", "agent-skill")
      const kxSkillsDir = path.join(dir, ".kx", "skills", "agent-skill")
      const claudeDir = path.join(dir, ".claude", "skills", "claude-skill")
      const agentDir = path.join(dir, ".agents", "skills", "agent-skill")
      await Bun.write(
        path.join(claudeDir, "SKILL.md"),
...
`,
      ...
      await Bun.write(
        path.join(kxSkillDir, "SKILL.md"),
        `---
name: kx-skill
description: A skill in the .kx/skill directory.
---
...
`,
      ...
      await Bun.write(
        path.join(kxSkillsDir, "SKILL.md"),
        `---
name: kx-skill
description: A skill in the .kx/skills directory.
---
