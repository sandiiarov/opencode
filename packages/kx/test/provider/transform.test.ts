...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    ...
    const result = ProviderTransform.message(msgs, kxModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.kx?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.kx?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    expect(result[0].providerOptions?.kx?.itemId).toBe("msg_kx")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.kx?.itemId).toBe("msg_kx_part")
...
describe("ProviderTransform.message - google vertex anthropic cache control", () => {
