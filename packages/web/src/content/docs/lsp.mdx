...
OpenCode integrates with your Language Server Protocol (LSP) to help the LLM interact with your codebase. It uses diagnostics to provide feedback to the LLM, including errors, warnings, info messages, and hints.
...
| stylelint          | .css, .pcss, .postcss, .scss, .sass, .less, .vue, .svelte, .astro   | Auto-installs for projects with stylelint config             |
...
3. Surfaces reported diagnostics grouped by severity so the model can react to errors, warnings, info, and hints.
