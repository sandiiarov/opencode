...
- [x] `c5442d418` effectify SessionPrompt service (partially integrated: local `packages/kx/src/session/prompt.ts` still uses the older `Instance.state` + `AbortController` orchestration instead of the upstream `Runner`/Effect service stack, so I skipped the broad service migration. The behavior-relevant piece that still applied on `my` was in `packages/kx/src/server/routes/session.ts`: the cancel endpoint now `await`s `SessionPrompt.cancel(...)` so the route does not return before session status cleanup runs. Added a regression check in `packages/kx/test/server/session-messages.test.ts` to lock that route shape in place.)
- [x] `567a91191` simplify LLM stream by replacing queue with fromAsyncIterable (skip: this cleanup only applies after the upstream Effect-layered `LLM.Service` stream wrapper added in `c5442d418`. Local `packages/kx/src/session/llm.ts` still exposes the older direct async `LLM.stream(...)` facade from the AI SDK, and `packages/kx/src/session/processor.ts` already consumes `result.fullStream` directly without the intermediate Effect `Queue`/`Stream.fromQueue` wrapper. There is no separate behavior fix to port on `my` until the broader service migration exists.)
- [x] `72c77d0e7` fix token usage double-counting (integrated: local `packages/kx/src/session/index.ts` still had the pre-AI-SDK-v6 Anthropic/Bedrock special case, which double-counted cached tokens after the SDK normalized `inputTokens` across providers. Ported the fix so `Session.getUsage(...)` always subtracts cached read/write tokens from `inputTokens` and now trusts `usage.totalTokens` directly; updated regression coverage in `packages/kx/test/session/compaction.test.ts` for Anthropic/Bedrock/Vertex Anthropic cache accounting.)
- [x] `5daf2fa7f` compaction agent responds in same language as conversation (skip: upstream change is prompt text only. It adds a single instruction line to `packages/opencode/src/agent/prompt/compaction.txt` and the inline fallback prompt in `packages/opencode/src/session/compaction.ts`, with no language-detection logic, no new data flow, and no tests. Local `packages/kx/src/agent/prompt/compaction.txt` has already been rewritten substantially for the continuation-checkpoint format, and local `packages/kx/src/session/compaction.ts` still builds its own custom `defaultPrompt` / plugin-overridable `promptText`. Since this fork intentionally changed compaction prompting, there is no reusable bug fix here to port unless you specifically want to add this exact instruction to your custom prompt.)
- [x] `196a03caf` discourage `_noop` tool call during LiteLLM compaction (partially integrated: local `packages/kx/src/session/compaction.ts` already had the upstream fallback-prompt instruction `Do not call any tools. Respond only with the summary text.`, so there was nothing to port there. The behavior-relevant remaining piece was the LiteLLM compatibility stub in `packages/kx/src/session/llm.ts`; I tightened the injected `_noop` tool description and schema so proxy backends still accept empty-tool compaction requests without encouraging the model to invoke the placeholder tool.)
- [x] `26cc924ea` prompt caching and cache token tracking for google-vertex-anthropic (integrated: local `packages/kx/src/provider/transform.ts` was still missing `google-vertex-anthropic` in the Anthropic-style cache-control branch, and local `packages/kx/src/session/index.ts` did not yet read cache-write tokens from the Vertex metadata key. Ported both pieces and added regression coverage in `packages/kx/test/provider/transform.test.ts` and `packages/kx/test/session/compaction.test.ts`.)
- [x] `4dd866d5c` transform fix after AI SDK v6 change (integrated: local `packages/kx/src/provider/transform.ts` still excluded `@ai-sdk/azure` from providerOptions key remapping even though AI SDK v6 now expects the `azure` key. Removed that obsolete exclusion and added regression coverage in `packages/kx/test/provider/transform.test.ts` for `azure-cognitive-services` remapping.)
- [x] `2daf4b805` dedicated system prompt for Kimi models (skip: upstream change is prompt-only. It adds a brand new `packages/opencode/src/session/prompt/kimi.txt` plus a single `SystemPrompt.provider(...)` selector branch. Local `packages/kx/src/session/prompt/default.txt` is already heavily customized for the `kx` agent contract, and importing upstream’s Kimi prompt verbatim would replace those fork-specific instructions rather than port a self-contained bug fix. There is no separate runtime/provider logic in this commit; if we ever want Kimi-specific tuning on `my`, it should be authored as a `kx`-native prompt variant instead of copied from upstream OpenCode text.)
